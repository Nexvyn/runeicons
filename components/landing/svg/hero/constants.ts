...
export const EMBERS = Array.from({ length: EMBERS_CFG.count }, (_, i) => {
  ...
  const dxJ = isOuter ? EMBERS_CFG.outerDriftXJitter : EMBERS_CFG.driftXJitter;
  return {
    id: `ember-${i}`,
    cx: round3(nozzleX + (emberRand(i, 1) - 0.5) * cxJ),
    cy: round3(478 + emberRand(i, 2) * 8),
    radius: round3(
      EMBERS_CFG.sizeMin + emberRand(i, 3) * (EMBERS_CFG.sizeMax - EMBERS_CFG.sizeMin),
    ),
...
export const CLOUDS = Array.from({ length: CLOUDS_CFG.count }, (_, i) => ({
  id: `cloud-${i}`,
  cx: round3(
    CLOUDS_CFG.spawnXMin + seededRand(i + 100, 1) * (CLOUDS_CFG.spawnXMax - CLOUDS_CFG.spawnXMin),
  ),
  cy: round3(CLOUDS_CFG.spawnY + (seededRand(i + 100, 2) - 0.5) * 12),
  radius: round3(
    CLOUDS_CFG.radiusMin + seededRand(i + 100, 3) * (CLOUDS_CFG.radiusMax - CLOUDS_CFG.radiusMin),
  ),
  delay: round3(i * CLOUDS_CFG.spawnStagger),
  cycle: round3(
    CLOUDS_CFG.cycleMin + seededRand(i + 100, 4) * (CLOUDS_CFG.cycleMax - CLOUDS_CFG.cycleMin),
  ),
  driftX: round3((seededRand(i + 100, 5) - 0.5) * 2 * CLOUDS_CFG.driftXJitter),
  driftY: round3(
    CLOUDS_CFG.driftYMin + seededRand(i + 100, 6) * (CLOUDS_CFG.driftYMax - CLOUDS_CFG.driftYMin),
  ),
}));
...
export const BLAST_ICONS = BLAST_ICON_SEATS.map((rawSeat, i) => {
  ...
  const seat = {
    ...
    y: round3(LAUNCH_PAD.cy + LAUNCH_PAD.ry * Math.sin(angle) - rawSeat.scale * 12 * 0.8),
  };
