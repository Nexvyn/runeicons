...
import { z } from "zod";
import { findIcon, getRelatedStyles, getSvgSource, getViewBox, ICON_COUNTS, listCategories, searchIcons, } from "./icons.js";
const STYLES = ["normal", "duotone", "fill", "pixelated", "glass"];
function iconToResult(icon, opts) {
...
function formatCategories() {
    ...
    for (const row of rows) {
        lines.push(`${row.category.padEnd(width)}  ${String(row.total).padStart(5)}  ${String(row.byStyle.normal).padStart(6)}  ${String(row.byStyle.duotone).padStart(6)}  ${String(row.byStyle.fill).padStart(4)}  ${String(row.byStyle.pixelated).padStart(9)}  ${String(row.byStyle.glass).padStart(5)}`);
    }
    return lines.join("\n");
}
async function main() {
    ...
    server.registerTool("search_icons", {
        ...
        description: "Search the Rune Icons library (900+ icons). Returns matching icons with id, name, style, category, and tags. " +
        ...
        const text = results.length === 0
            ...
            : [
                ...
                "",
                ...results.map((icon) => `- ${icon.id} — \"${icon.name}\" (style: ${icon.style}, category: ${icon.category})`),
                "",
    ...
    server.registerTool("get_icon", {
        ...
        description: 'Get full metadata and ready-to-use SVG source for a Rune Icon by id (e.g. "arrows-arrow-down-left", "glass-archive"). ' +
        ...
        if (result.relatedStyles) {
            lines.push(`other styles: ${result.relatedStyles.map((r) => `${r.style} (${r.id})`).join(", ")}`);
        }
    ...
    server.registerTool("list_categories", {
        ...
        description: "List every Rune Icons category with total and per-style counts. Use a category name to filter search_icons results.",
        inputSchema: {},
    }, async () => {
